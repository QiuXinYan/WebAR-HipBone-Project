...
import { UIPanel, UIBreak, UIRow, UIColor, UISelect, UIText, UINumber, UIButton, UICheckbox } from './libs/ui.js';
...
function SidebarScene(editor) {
	...
	console.log(editor.uniqueObject)
	...
	buttonCreateMarketObject.onClick(function () {
		...
		var newObjects = originObject.clone();
	...
	createMarkerObjectsRow.add(buttonCreateMarketObject);
	container.add(createMarkerObjectsRow);


	//button store matrix info
	...
	const storeMatrixRow = new UIRow();
	const matrixStoreButton = new UIButton().setLeft('50px');
	matrixStoreButton.onClick(function () {
		var object = editor.selected;
		console.log(object)
		var data = {
			id: editor.storeCounts,
			...
			; (async function () {
				var info = await IndexDB.addData(db, storeName, data);
				console.log(info);
			})();
		editor.storeCounts++;
		editor.signals.storeinfoChange.dispatch();
	});
	storeMatrixRow.add(new UIText(strings.getKey('sidebar/scene/storeMatrix')).setWidth('90px'));
	storeMatrixRow.add(matrixStoreButton);
	container.add(storeMatrixRow);
	...
	function refreshUI() {
		...
		//options.push(buildOption(editor.uniqueObject, true))
